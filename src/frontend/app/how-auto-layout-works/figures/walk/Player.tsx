...
export default function Player({ frames, render, caption, stepMs = 1100 }: { frames: LabFrame[]; render: (f: LabFrame) => ReactNode; caption?: ReactNode; stepMs?: number }) {
  ...
  return (
    <figure className="my-6 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800/40 p-3 sm:p-4">
      {frames.length > 1 && <div className="flex flex-wrap items-center gap-2 mb-3">
        <button className={btn} onClick={() => { setPlaying(false); setI((k) => Math.max(0, k - 1)); }} disabled={i === 0}>&larr;</button>
        <button className={btn} onClick={() => { setPlaying(false); setI((k) => Math.min(frames.length - 1, k + 1)); }} disabled={i >= frames.length - 1}>&rarr;</button>
        <button className={btn} onClick={() => { if (i >= frames.length - 1) setI(0); setPlaying((p) => !p); }}>{playing ? "Pause" : i >= frames.length - 1 ? "Replay" : "Play"}</button>
        <button className={btn} onClick={() => { setPlaying(false); setI(0); }} disabled={i === 0}>Restart</button>
